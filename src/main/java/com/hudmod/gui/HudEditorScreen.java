...
/**
 * HUD Editor Screen — opened by the configurable keybind.
 *
 * Modes:
 *  • SETTINGS  — toggle features, open edit mode, save, reset.
 *  • EDIT      — drag HUD elements freely; right-click for per-element popup.
 *
 * Performance:
 *  • No widget allocations per frame; buttons are drawn as plain fills.
 *  • Popup menus created on demand, discarded on close.
 *  • HUD elements are rendered live in the background so the player sees
 *    changes immediately.
 */
public class HudEditorScreen extends Screen {
    ...
    private static final class DragTarget {
        ElementConfig cfg;
        String        label;
        int           boxW, boxH;   // pixel dims for hit-test & outline
    ...
    // ── Button hit regions (set in render, used in click) ────────────────────
    // Stored as [x,y,w,h] per button row; indexed by BTN_* constants below
    ...
    @Override
    public boolean shouldPause() { return false; } // keep game ticking
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        // Do NOT call super.render — it draws the dark vanilla overlay which we replace
    }
    ...
    private void renderSettingsPanel(DrawContext ctx, int mouseX, int mouseY) {
        ...
        int panelH = PAD + 10 + PAD + BTN_TOTAL * ROW_H + PAD + ROW_H + PAD;
    ...
    private int drawToggleRow(DrawContext ctx, int mouseX, int mouseY,
        ...
        int rowBg     = hover ? 0x20FFFFFF : 0;
        ctx.fill(px + 1, ry, px + PANEL_W - 1, ry + ROW_H - 1, rowBg);
    ...
    private void renderEditMode(DrawContext ctx, int mouseX, int mouseY) {
        // Build drag targets on first render in edit mode
        ...
        // Draw outlines around each element
    ...
    private void buildDragTargets() {
        ...
        // Approximate box sizes (must match ArmorHud logic)
        for (int i = 0; i < 4; i++) {
    ...
    public boolean mouseClicked(double mx, double my, int button) {
        ...
        if (editMode) {
            ...
            } else if (button == 0) { // left-click → start drag
                for (DragTarget t : dragTargets) {
                    int ex = (int)(t.cfg.xFraction * width);
                    int ey = (int)(t.cfg.yFraction * height);
                    if (x >= ex && x <= ex+t.boxW && y >= ey && y <= ey+t.boxH) {
    ...
    private void handleSettingsClick(int x, int y) {
        ...
        if (isOverBtn(BTN_EDIT,  x, y)) {
            editMode = true;
            dragTargets.clear(); // rebuild on next render
        }
    ...
    public boolean mouseDragged(double mx, double my, int button, double dX, double dY) {
        ...
        if (dragging != null) {
            float nx = (float)(x - dragOffX) / width;
            float ny = (float)(y - dragOffY) / height;
            dragging.cfg.xFraction = Math.max(0f, Math.min(1f, nx));
            dragging.cfg.yFraction = Math.max(0f, Math.min(1f, ny));
            return true;
        }
        return false;
    }
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        // ESC in edit mode → go back to settings panel
